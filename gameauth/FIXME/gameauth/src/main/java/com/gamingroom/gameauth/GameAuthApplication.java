...
public class GameAuthApplication extends Application<Configuration> {
	...
	public void run(Configuration c, Environment e) throws Exception 
	{
		...
		// build the HTTP client first
		Client client = new JerseyClientBuilder(environment).build("DemoRESTClient");

		// register the client controller using that client
		environemnt.jersey().register(new RESTClientController(client));
